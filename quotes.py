...
async def get_quote(root):
    ...
    last = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Last")]/following-sibling::td/text()'))
    d['Last'] = last

    change = extract_first(quote_table_ele.xpath('.//tr/td/font/text()'))
    d['Change'] = change

    last_trade = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Last Trade")]/following-sibling::td/text()'))
    d['Last Trade'] = last_trade

    open_ = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Open")]/following-sibling::td/text()'))
    d['Open'] = open_

    bid = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Bid")]/following-sibling::td/text()'))
    d['Bid'] = bid

    tick_vol = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Tick Vol")]/following-sibling::td/text()'))
    d['Tick Vol'] = tick_vol

    high = extract_first(quote_table_ele.xpath('.//td[contains(text(),"High")]/following-sibling::td/text()'))
    d['High'] = high

    ask = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Ask")]/following-sibling::td/text()'))
    d['Ask'] = ask

    volume = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Volume")]/following-sibling::td/text()'))
    d['Volume'] = volume

    low = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Low")]/following-sibling::td/text()'))
    d['Low'] = low

    exchange = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Exchange")]/following-sibling::td/text()'))
    d['Exchange'] = exchange

    open_int = extract_first(quote_table_ele.xpath('.//td[contains(text(),"Open Int")]/following-sibling::td/text()'))
    d['Open Int'] = open_int
...
async def quote_to_csv(client, endpoint, save_path, sleep_time=0):
    ...
    with save_path.open(mode='a+') as outfile:
        outfile.write(f"{d['Datetime']},{d['Last']},{d['Change']},{d['Last Trade']},{d['Tick Vol']},{d['Volume']},{d['Open']},{d['Bid']},{d['High']},{d['Ask']},{d['Low']},{d['Open Int']},{d['Exchange']}\n")
    await asyncio.sleep(sleep_time)
    loop.create_task(
        quote_to_csv(client, endpoint, save_path, sleep_time))
...
async def main(loop, symbol_to_save_path):
    ...
    for symbol, save_path in symbol_to_save_path.items():
        ...
        loop.create_task(
            quote_to_csv(client, endpoint, save_path))
